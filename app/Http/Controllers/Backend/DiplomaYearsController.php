...
class DiplomaYearsController extends Controller
{
 
    public function index()
    {
        $curriculum = Diploma::wherenull('deleted_by')->get();
        return view('backend.academics.diploma.index', compact('curriculum'));
    }

    public function create(Request $request)
    {
        return view('backend.academics.diploma.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        $validatedData = $request->validate([
            ...
            'framework_heading'       => 'nullable|string|max:255',
        ...
        return redirect()->route('manage-diploma.index')
            ->with('message', 'Program details have been successfully saved!');
    }
    
    public function edit($id)
    {
        $curriculum = Diploma::findOrFail($id);
        return view('backend.academics.diploma.edit', compact('curriculum'));
    }

    public function update(Request $request, $id)
    {
        $pyp = Diploma::findOrFail($id);
    ...
    public function destroy(string $id)
    {
        ...
        try {
            $industries = Diploma::findOrFail($id);
            $industries->update($data);

            return redirect()->route('manage-diploma.index')->with('message', 'Details deleted successfully!');
        } catch (Exception $ex) {
