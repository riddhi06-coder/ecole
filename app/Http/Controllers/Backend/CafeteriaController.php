...
class CafeteriaController extends Controller
{
    ...
    {
        $cafeteria = Cafeteria::orderBy('id', 'asc')->wherenull('deleted_by')->get();
        return view('backend.campus.cafeteria.index', compact('cafeteria'));
    }

    public function create(Request $request)
    {
        return view('backend.campus.cafeteria.create');
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $validated = $request->validate([
                ...
                'bck_image'    => 'required|image|mimes:jpg,jpeg,png,webp,svg|max:2048',
            ...
            $bckImageName = null;
            if ($request->hasFile('bck_image')) {
                $bckImage = $request->file('bck_image');
                $bckImageName = time() . '_' . rand(10, 999) . '.' . $bckImage->getClientOriginalExtension();
                $bckImage->move(public_path('uploads/campus-life'), $bckImageName);
            }

            // ✅ Save Record to Database
            $sports = new Cafeteria(); // Make sure the model exists
            ...
            return redirect()->route('manage-cafeteria.index')
    ...
    {
        $sports = Cafeteria::findOrFail($id);
        return view('backend.campus.cafeteria.edit', compact('sports'));
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            // Handle Bck Image Upload
            if ($request->hasFile('bck_image')) {
                if ($sports->bck_image && file_exists(public_path('uploads/campus-life/' . $sports->bck_image))) {
                    unlink(public_path('uploads/campus-life/' . $sports->bck_image));
                }
            ...
            return redirect()->route('manage-cafeteria.index.index')
            ...
            Log::error('Error updating Cafeteria record', [
    ...
    public function destroy(string $id)
    {
        ...
        try {
            $industries = Cafeteria::findOrFail($id);
            $industries->update($data);

            return redirect()->route('manage-cafeteria.index')->with('message', 'Details deleted successfully!');
        } catch (Exception $ex) {
