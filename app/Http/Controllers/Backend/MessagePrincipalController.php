...
class MessagePrincipalController extends Controller
{

    public function index()
    {
        $records = MessagePrincipal::wherenull('deleted_by')->get();
        return view('backend.about.message_principal.index', compact('records'));
    }

    public function create(Request $request)
    {
        return view('backend.about.message_principal.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('manage-message-from-principal.index')->with('message', 'Details successfully added.');
    }

    public function edit($id)
    {
        $set_apart = MessagePrincipal::findOrFail($id);
        return view('backend.about.message_principal.edit', compact('set_apart'));
    }

    public function update(Request $request, $id)
    {
        // Find the record
        $setApart = MessagePrincipal::findOrFail($id);
        ...
        return redirect()->route('manage-message-from-principal.index')->with('message', 'Details successfully updated.');
    }
    ...
    public function destroy(string $id)
    {
        ...
        try {
            $industries = MessagePrincipal::findOrFail($id);
            $industries->update($data);

            return redirect()->route('manage-message-from-principal.index')->with('message', 'Details deleted successfully!');
        } catch (Exception $ex) {
