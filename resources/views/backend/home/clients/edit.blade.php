...
<head>
    @include('components.backend.head')
                            ...
                            <div class="tab-content" id="wizard-tabContent">
                                <div class="tab-pane fade show active" id="wizard-contact" role="tabpanel" aria-labelledby="wizard-contact-tab">
                                    <form class="row g-3 needs-validation custom-input" novalidate 
                                        action="{{ route('manage-clients.update', $clients->id) }}" 
                                        ...
                                        <div class="col-12">
                                            ...
                                            <table class="table table-bordered mt-5" id="featuresTable">
                                                ...
                                                <tbody>
                                                    ...
                                                    @if($clients->clients)
                                                        @foreach(json_decode($clients->clients, true) as $feature)
                                                            <tr class="feature-row">
                                                                <td>
                                                                    <input type="file" name="features[{{ $featureIndex }}][image]" 
                                                                        class="form-control feature-image" 
                                                                        onchange="previewFeatureImage(event, this)">
                                                                    <input type="hidden" name="features[{{ $featureIndex }}][existing_image]" 
                                                                        value="{{ $feature['image'] }}">
                                                                    <small class="text-secondary"><b>Note: The file size should be less than 2MB.</b></small><br>
                                                                    <small class="text-secondary"><b>Only .jpg, .jpeg, .png, .webp, .svg allowed.</b></small>
                                                                </td>
                                                                <td>
                                                                    <img src="{{ asset('uploads/home/'.$feature['image']) }}" 
                                                                        class="img-fluid mt-2 feature-preview" 
       ...
       @include('components.backend.main-js')
        ...
        <script>
            let featureIndex = {{ $featureIndex }}; // start from last existing index
    ...
    // Add new row (works for any button with class .add-feature)
